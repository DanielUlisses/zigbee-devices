...
class EnergyReportCoordinator:
    ...
    async def _load_solar_data(self) -> None:
        ...
        billing_data = self.hass.data[DOMAIN][self.config_entry.entry_id]["billing_data"]
        
        if not billing_data:
            return
            
        # Find earliest and latest dates
        dates = [datetime.fromisoformat(date_str).date() for date_str in billing_data.keys()]
        if not dates:
            return
            
        start_date = min(dates) - timedelta(days=self.config_entry.data.get(CONF_BILLING_PERIOD_DAYS, DEFAULT_BILLING_PERIOD_DAYS))
        end_date = max(dates)
    ...
    async def _calculate_periods(self) -> None:
        ...
        periods = {}
        cumulative_balance = self.config_entry.data.get(CONF_INITIAL_BALANCE, DEFAULT_INITIAL_BALANCE)
        
        prev_grid_consumption = self.config_entry.data.get(CONF_INITIAL_GRID_CONSUMPTION, 0.0)
        prev_grid_injection = self.config_entry.data.get(CONF_INITIAL_GRID_INJECTION, 0.0)
        prev_end_date = None
        ...
        for end_date_str, data in sorted_periods:
            ...
            if prev_end_date:
                ...
                billing_period_days = self.config_entry.data.get(CONF_BILLING_PERIOD_DAYS, DEFAULT_BILLING_PERIOD_DAYS)
                start_date = end_date - timedelta(days=billing_period_days - 1)
            
            # Calculate grid consumption and injection for this period
            grid_consumption_period = data["grid_consumption_reading"] - prev_grid_consumption
            grid_injection_period = data["grid_injection_reading"] - prev_grid_injection
            
            # Calculate solar generation for this period
            solar_generation_period = self._calculate_solar_for_period(start_date, end_date)
            
            # Calculate derived metrics
            total_consumption = grid_consumption_period + (solar_generation_period - grid_injection_period)
            ...
            minimum_kwh = self.config_entry.data.get(CONF_MINIMUM_BILLING_KWH, DEFAULT_MINIMUM_BILLING_KWH)
    ...
    def _calculate_solar_for_period(self, start_date: date, end_date: date) -> float:
        ...
        manual_solar_data = self.hass.data[DOMAIN][self.config_entry.entry_id]["manual_solar_data"]
...
class EnergyReportSensor(SensorEntity, RestoreEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        self._attr_native_value = self._coordinator.get_current_period_value(self._sensor_type)
